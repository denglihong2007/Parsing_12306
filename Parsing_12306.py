    ...
    for dct in d:
        ...
        if ('station_name' in dct and dct['station_name'] in station_lists):
            ...
            if (('start_station_name' in dct) and rules_list[0] == dct['start_station_name']):
                ...
                if (rules_list[2] != '0'):
                    Time = datetime.datetime.strptime(dct['arrive_time'], "%H:%M")
                    dct['arrive_time'] = (Time + datetime.timedelta(minutes=int(rules_list[2]))).strftime("%H:%M")
                if (rules_list[3] != '0'):
                    Time = datetime.datetime.strptime(dct['start_time'], "%H:%M")
                    dct['start_time'] = (Time + datetime.timedelta(minutes=int(rules_list[3]))).strftime("%H:%M")
                print(dct)
        ...
        if ('start_station_name' in dct and dct['start_station_name'] in station_lists):
            rules_list = rules_lists[station_lists.index(dct['start_station_name'])]
            if (('start_station_name' in dct) and rules_list[0] == dct['start_station_name']):
                dct['start_station_name'] = rules_list[1]
                if (rules_list[2] != '0'):
                    Time = datetime.datetime.strptime(dct['arrive_time'], "%H:%M")
            ...
            if (('station_name' in dct) and rules_list[0] == dct['station_name']):
                ...
                if (rules_list[3] != '0'):
                    ...
                    dct['start_time'] = (Time + datetime.timedelta(minutes=int(rules_list[3]))).strftime("%H:%M")
                print(dct)
        train.addStation(dct['station_name'],dct['arrive_time'],dct['start_time'],business=True)
...
headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.99 Safari/537.36 Edge/97.0.1072.69'}
chinese = "[A-Za-z0-9\!\%\[\]\,\。\"\:\_]"
...
while True:
    ...
    start_station_code = telecode_list[telecode_list.find(start_station + "|") + len(start_station) + 1:telecode_list.find(start_station + "|") + len(start_station) + 4]
    to_station_code = telecode_list[telecode_list.find(to_station + "|") + len(to_station) + 1:telecode_list.find(to_station + "|") + len(to_station) + 4]

    year = date[0:4]
    month = date[4:6]
    day = date[6:]

    url = ("https://kyfw.12306.cn/otn/czxx/queryByTrainNo?train_no={train_no}&from_station_telecode={start_station_code}&to_station_telecode={to_station_code}&depart_date={year}-{month}-{day}").format(train_no = train_no,start_station_code = start_station_code,to_station_code = to_station_code,year = year,month = month,day = day)

    print ("查询到" + search_train + "的代码为" + train_no + "，起点站为" + start_station + "，终点站为" + to_station + "，起点电报码为" + start_station_code + "，终点电报码为" + to_station_code + "。")
